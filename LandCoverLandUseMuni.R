...
muni <- st_read("D:/Dropbox/Forest Composition/composition/Maps/shapefiles/MunisMinusWill.shp") %>% 
...
target <- muni[1,'NAME'] #Change first value to run another muni

area <- muni[1,'areaAc'] #Make sure first value is the same as above

#create folder to catch figures

dir.create(paste0('D:/Dropbox/chicago region trees initiative/Data and Maps/CanopyRecommendations/MunicipalCanopySummaries/2022/', target))
...
setwd(paste0('D:/Dropbox/chicago region trees initiative/Data and Maps/CanopyRecommendations/MunicipalCanopySummaries/2022/', target))
...
pPlantLong <- pPlant %>% group_by(Type) %>% 
  ...
  mutate(sort = rep(c(3,2,1), each = 11), #Add column to sort chart
         value = `value` * 100) 
